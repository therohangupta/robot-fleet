...
from ..config import REPO_ROOT, EMBODIMENTS_DIR, DEFAULT_ROBOT_HOST, DEFAULT_ROBOT_BASE_PORT
...
async def check_robot_health_endpoint(
    ...
    By default uses Telemetry service (heartbeat-based). Pass source=direct to
    ping the robot's HTTP endpoint directly (legacy/debugging).
    """
    robot = bridge.get_robot_status(robot_id)
    if not robot:
        raise HTTPException(status_code=404, detail=f"Robot {robot_id} not found")
    ...
    if source == "direct":
        ...
        return {"robot_id": robot_id, "host": host, "port": port, "source": "direct", **health}

    telemetry_health = await get_telemetry_robot_health(robot_id)
...
async def check_all_robots_health_endpoint(
    ...
    By default uses Telemetry service (heartbeat-based). Pass source=direct to
    ping each robot's HTTP endpoint directly (legacy/debugging).
    """
    robots = bridge.list_robots("all")

    if source == "direct":
        results = await check_all_robots_health(robots)
        return {"robots": results, "source": "direct"}

    # Telemetry stores health keyed only by host:port. We join with fleet registry (robot_id + host/port) here.
    ...
    for robot in robots:
        ...
        if th:
            # Preserve fleet robot_id; telemetry's "robot_id" is the host:port key
            results.append({
                **th,
                "robot_id": robot_id,
                "host": host,
    ...
    return {"robots": results, "source": "telemetry"}
...
async def get_robot_allocations(
    ...
    try:
        ...
        logger.error("Error getting allocations for robot %s: %s", robot_id, e, exc_info=True)

...
class BasePlanner(ABC):
    ...
    async def save_plan_to_db(self, plan_json: str, planning_strategy: int, allocation_strategy: int, goal_ids: List[int],
        ...
        try:
            ...
            print(f"🎯 PLAN CREATION: Saving plan with prompts keys: {list(self.planning_prompts.keys()) if self.planning_prompts else 'None'}")
            print(f"🎯 PLAN CREATION: Saving plan with artifacts keys: {list(self.planning_artifacts.keys()) if self.planning_artifacts else 'None'}")
            print(f"🎯 PLAN CREATION: Saving plan with {len(self.server_logs) if self.server_logs else 0} log entries")
